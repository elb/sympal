...
class Basesympal_editorActions extends sfActions
{
  ...
  public function executePublish_content(sfWebRequest $request)
  {
    $this->askConfirmation(
      __('Publish Content'),
      __('Are you sure you want publish this content?')
    );

    $this->getRoute()->getObject()->publish();

    $this->getUser()->setFlash('notice', __('Content published successfully!'));
  ...
  {
    $this->askConfirmation(
      __('Un-publish Content'),
      __('Are you sure you want un-publish this content?')
    );

    $this->getRoute()->getObject()->unpublish();
    
    $this->getUser()->setFlash('notice', __('Content un-published successfully!'));
    $this->redirect($request->getParameter('redirect_url'));
  }
