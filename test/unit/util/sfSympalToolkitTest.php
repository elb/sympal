...
$t->info('1 - Make a call to the test/test partial. It prints out the value of $var');
...
$t->info('2 - Test the getContentRoutesYaml() method');
  ...
  $type = Doctrine_Core::getTable('sfSympalContentType')->findOneByKey('product');
  ...
  test_content_route($t, $routes['sympal_content_testing_product'], $type, '/product/testing-product', array('module' => 'product', 'sympal_content_id' => $content->id));
  ...
  $routes = sfYaml::load(sfSympalToolkit::getContentRoutesYaml());
  $t->is(count($routes), 2, 'Two routes are created for the one content type and one custom content');
  test_content_route($t, $routes['sympal_content_testing_product'], $type, '/product/testing-product', array('action' => 'customAction', 'sympal_content_id' => $content->id));
